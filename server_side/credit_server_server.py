...
# AI persona prompts (synced from client dy/ai_reply_agent.py)
# 注意：必须与本地 _PERSONA_PROMPTS 完全一致，否则云端生成的评论风格会与本地模式不符。
PERSONA_PROMPTS = {
    "a_zhen": {
        "video_comment": (
...
def generate_video_comment(req: AIRequest, request: Request, lic: sqlite3.Row = Depends(require_license)):
    ...
    machine_id = register_activation(lic["license_key"], req, request)
    # 同步本地 _human_prompt 格式（ai_reply_agent.py:545）：仅提供上下文，不带指令句
    keyword_text = req.keyword or "未提供"
    prompt = f"标题：{req.title}\n搜索关键词：{keyword_text}"
    try:
        ensure_min_balance(lic["license_key"], machine_id=machine_id)  # F2: 预检机器余额，避免白调上游 LLM
        resp = create_chat([
            {"role": "system", "content": PERSONA_PROMPTS.get(req.persona, PERSONA_PROMPTS["a_zhen"])["video_comment"]},
            {"role": "user", "content": prompt},
        ], temperature=1.1, max_tokens=200)
        message = resp.choices[0].message if resp.choices else None
        # 与本地一致：AI 返回空则 reply 为空字符串，客户端 _is_valid_reply 校验失败后跳过发送
        text = first_nonempty_line(message.content if message else "", "", 80)
        usage = resp.usage
...
def generate_lead_reply(req: AIRequest, request: Request, lic: sqlite3.Row = Depends(require_license)):
    check_rate_limit(request, "ai", 600)
    machine_id = register_activation(lic["license_key"], req, request)
    # 同步本地 HumanMessage 格式（ai_reply_agent.py:511）：仅提供上下文，不带指令句
    prompt = (
        f"视频标题：{req.title or '未提供'}\n"
        f"搜索关键词：{req.keyword or '未提供'}\n"
        f"用户评论：{req.comment_text}"
    )
    try:
        ensure_min_balance(lic["license_key"], machine_id=machine_id)  # F2: 预检机器余额，避免白调上游 LLM
        resp = create_chat([
            {"role": "system", "content": PERSONA_PROMPTS.get(req.persona, PERSONA_PROMPTS["a_zhen"])["lead_reply"]},
            {"role": "user", "content": prompt},
        ], temperature=1.1, max_tokens=80)
        message = resp.choices[0].message if resp.choices else None
        # 与本地一致：AI 返回空则 reply 为空字符串，客户端 _is_valid_lead_reply 校验失败后跳过发送
        text = first_nonempty_line(message.content if message else "", "", 100)
        usage = resp.usage
