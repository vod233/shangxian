...
def render_execution_functions():
    render_page_header("AI员工功能管道编排", "按视频执行顺序排列，勾选要启用的步骤，预设一键切换。")

    fetch_config()
    config = st.session_state.config_data

    # Phase2: 加载管道步骤元数据
    steps_meta = _fetch_pipeline_steps()

    # 从配置或元数据读取当前每个步骤的启用状态
    saved_steps = config.get("pipeline_steps", {})
    step_states = {}
    for s in steps_meta:
        sid = s["id"]
        step_states[sid] = saved_steps.get(sid, s.get("default_enabled", True))

    with st.form("execution_form", border=False):
        st.markdown('<div class="card">', unsafe_allow_html=True)

        # 预设快捷按钮
        st.markdown('<div style="margin-bottom:10px; font-size:12px; color:#9CA3AF;">预设模板</div>', unsafe_allow_html=True)
        c1, c2, c3, c4 = st.columns(4)
        with c1:
            if st.form_submit_button("全功能", use_container_width=True):
                step_states = {s["id"]: True for s in steps_meta}
        with c2:
            if st.form_submit_button("仅获客", use_container_width=True):
                step_states = {s["id"]: s["category"] in ("fixed", "acquisition") for s in steps_meta}
        with c3:
            if st.form_submit_button("仅互动", use_container_width=True):
                step_states = {s["id"]: s["category"] in ("fixed", "content") for s in steps_meta}
        with c4:
            if st.form_submit_button("仅点赞", use_container_width=True):
                step_states = {s["id"]: s["category"] == "fixed" for s in steps_meta}

        st.markdown('<div style="height:1px; background:rgba(255,255,255,0.06); margin:12px 0;"></div>', unsafe_allow_html=True)

        # 分类标签颜色映射
        # 分类标签颜色映射（hardcoded rgba）
        cat_styles = {
            "fixed":      {"color": "#6366F1", "bg": "rgba(99,102,241,0.06)"},
            "acquisition": {"color": "#D85A30", "bg": "rgba(216,90,48,0.06)"},
            "content":    {"color": "#1D9E75", "bg": "rgba(29,158,117,0.06)"},
        }
        cat_labels = {
            "fixed": "固定",
            "acquisition": "私域获客",
        ...
        # 按分类分组渲染步骤
        for cat in ["fixed", "acquisition", "content"]:
            cat_steps = [s for s in steps_meta if s["category"] == cat]
            if not cat_steps:
                continue

            style = cat_styles.get(cat, {"color": "#888780", "bg": "rgba(136,135,128,0.06)"})
            label = cat_labels.get(cat, cat)
            st.markdown(f"""
            <div style="padding:6px 10px; margin-bottom:4px; background:{style['bg']}; border-left:3px solid {style['color']}; border-radius:4px; font-size:11px; color:{style['color']};">
                {label}
            </div>
            """, unsafe_allow_html=True)
            ...
            for s in cat_steps:
                sid = s["id"]
                checked = step_states.get(sid, s.get("default_enabled", True))
                disabled = sid == "like"  # 点赞不可取消
                step_states[sid] = st.checkbox(
                    f"{s['label']}",
                    value=checked,
                    disabled=disabled,
                    help=f"{s.get('description', '')} | 限额: {', '.join(s.get('quota_keys', []))} | 概率决策: {'是' if s.get('probability_key') else '否'}",
                )

        st.markdown("</div>", unsafe_allow_html=True)

        # 概率决策总开关
        st.markdown('<div style="height:1px; background:rgba(255,255,255,0.06); margin:14px 0;"></div>', unsafe_allow_html=True)
        ...
        <div style="margin-top:12px; padding:10px 14px; background:rgba(99,102,241,0.08); border-left:3px solid #6366F1; border-radius:6px; font-size:12px; color:#9CA3AF; line-height:1.6;">
            <div style="font-weight:600; color:#A5B4FC; margin-bottom:4px;">执行说明</div>
            <div>• <b>点赞</b>：每视频固定优先，受概率与每日80次限额控制</div>
            <div>• <b>私域获客</b>：进主页→关注→私信，粉丝/私信阈值见"私信话术调整"页。执行失败自动阻断后续</div>
            <div>• <b>AI视频评论</b>：AI生成文本→快捷输入框发布，<b>不离开视频页</b></div>
            <div>• <b>评论区截流</b>：打开评论区→AI扫描意向→发送楼中楼回复，与视频评论共享每日50条限额</div>
            <div>• 执行顺序: <b>点赞 → 私域获客 → AI视频评论 → 评论区截流</b></div>
            <div>• 任一私域获客或评论区截流失败会阻断后续步骤</div>
        </div>
        """, unsafe_allow_html=True)

        if st.form_submit_button("💾 保存管道配置", use_container_width=True, type="primary"):
            # 校验：至少开启一个非点赞步骤
            non_like_enabled = any(
                step_states.get(s["id"], False)
                for s in steps_meta if s["category"] != "fixed"
            )
            if not non_like_enabled:
                st.error("请至少开启一个功能步骤，否则任务将仅点赞无业务产出")
            else:
                # 构建 pipeline_steps
                pipeline_steps = {sid: val for sid, val in step_states.items()}

                # 向后兼容：同步 mode-level 字段
                acq_on = step_states.get("acquisition", False)
                content_on = step_states.get("video_comment", False) or step_states.get("comment_lead", False)

                payload = {
                    "enable_like": True,
                    "enable_mode_customer_acquisition": acq_on,
                    "enable_mode_content_interaction": content_on,
                    "enable_anti_detection_probability": enable_anti_detection_probability,
                    "pipeline_steps": pipeline_steps,
                    "enable_author_follow": acq_on,
                    "enable_private_message": acq_on,
                    "enable_video_comment": content_on,
                    "enable_comment_lead": content_on,
                }
                _save_config(payload, "管道配置")


def _fetch_pipeline_steps():
    """从后端 API 获取管道步骤元数据"""
    try:
        resp = requests.get(f"{API_BASE_URL}/pipeline/steps", timeout=5)
        if resp.status_code == 200:
            data = resp.json()
            if data.get("success"):
    ...
    # 降级：返回硬编码默认值
    return [
        {"id": "like", "label": "点赞", "description": "每视频点右侧心形按钮", "category": "fixed", "default_enabled": True, "quota_keys": ["like"], "probability_key": "like"},
        {"id": "acquisition", "label": "私域获客", "description": "进主页→关注→私信", "category": "acquisition", "default_enabled": True, "quota_keys": ["follow", "private_message"], "probability_key": "follow"},
        {"id": "video_comment", "label": "AI视频评论", "description": "AI生成评论→发布", "category": "content", "default_enabled": True, "quota_keys": ["comment"], "probability_key": "comment"},
        {"id": "comment_lead", "label": "评论区截流", "description": "扫描评论区→回复意向评论", "category": "content", "default_enabled": True, "quota_keys": ["comment"], "probability_key": "comment_lead"},
    ]


def _save_config(payload, label="配置"):
    """通用配置保存（含乐观锁）"""
    try:
        current_config = {}
        resp = requests.get(f"{API_BASE_URL}/config?platform=douyin")
