...
# Phase2: 管道步骤元数据
from dy.pipeline import get_pipeline_step_info
...
def _infer_pipeline_steps(data):
    """Phase2: 从配置推断按步骤粒度的开关状态。
    优先读直接存储的 pipeline_steps，缺失时从 mode-level 字段推断。"""
    interaction = data.get("interaction", {})
    explicit = interaction.get("pipeline_steps")
    if explicit and isinstance(explicit, dict) and explicit:
        return dict(explicit)

    # 向后兼容：从 mode-level 字段推断
    mode1 = interaction.get("enable_mode_customer_acquisition")
    mode2 = interaction.get("enable_mode_content_interaction")
    if mode1 is None:
        mode1 = interaction.get("enable_author_follow", True) or interaction.get("enable_private_message", True)
    if mode2 is None:
        mode2 = interaction.get("enable_video_comment", True) or interaction.get("enable_comment_lead", True)

    return {
        "like": True,                         # 点赞始终固定
        "acquisition": bool(mode1),           # mode1 → 私域获客
        "video_comment": bool(mode2),         # mode2 → AI视频评论
        "comment_lead": bool(mode2),          # mode2 → 评论区截流
    }
...
def _load_douyin_config_for_frontend():
    ...
    return {
        ...
        # Phase2: 按步骤粒度的开关；缺失时按 mode-level 推断
        "pipeline_steps": _infer_pipeline_steps(data),
...
def _save_douyin_config(config: AppConfig):
    ...
    user_yaml_data = {
        ...
        "interaction": {
            ...
            # Phase2: 按步骤粒度的开关
            "pipeline_steps": config.pipeline_steps or {},
...
@app.get("/api/pipeline/steps", summary="获取管道步骤信息")
def api_pipeline_steps():
    """
    Phase2: 返回管道步骤元数据 + 当前启用状态，供前端渲染复选框
    """
    try:
        steps_info = get_pipeline_step_info()
        data = _load_douyin_config_for_frontend()
        current_steps = data.get("pipeline_steps", {})
        for info in steps_info:
            step_id = info["id"]
            info["enabled"] = current_steps.get(step_id, info["default_enabled"])
        return {"success": True, "steps": steps_info}
    except Exception as e:
        logging.warning(f"加载管道步骤信息失败: {e}")
...
def api_save_config(config: AppConfig, platform: str = "douyin"):
    ...
    try:
        ...
        # 双模式全关校验：至少开启一个模式，避免任务空转浪费流量
        if not config.enable_mode_customer_acquisition and not config.enable_mode_content_interaction:
            return {
                "success": False,
                "message": "请至少开启一个互动模式（模式1或模式2），否则任务将仅点赞无业务产出",
            }
