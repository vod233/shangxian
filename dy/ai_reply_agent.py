...
class DYReplyAgent:
    ...
    def _sanitize_reply(self, text: Optional[str]) -> Optional[str]:
        ...
        # 截断：尽量在自然位置断开，避免切断中文字符
        if len(cleaned) > 38:
            for cut_marker in ("。", "，", "、", "…"):
                cut_at = cleaned.rfind(cut_marker, 20, 38)
                if cut_at > 0:
                    cleaned = cleaned[:cut_at + 1]
                    break
            else:
                cleaned = cleaned[:36]
        return cleaned
    ...
    def _local_intent_guess(self, text: str, custom_keywords: list = None) -> bool:
        ...
        default_pattern = (
            r"(怎么买|去哪里|哪里买|在哪|哪里|求|想要|想买|想了解|了解|问一下"
            r"|价格|多少钱|多少|贵吗|推荐|教程|方法|链接|资料|怎么做"
            r"|靠谱吗|靠谱不|有用吗|好用吗|好用不|有效吗|有用不|怎么样|真的吗|真的假的"
            r"|能行吗|可以吗|效果|值不值|值得|划[算得]|在吗|有人|有买|买过|用过|试过"
            r"|求带|带上|上车|蹲|等一个)"
        )
    ...
    def _build_lead_fallback(self, comment_text: str) -> str:
        ...
        if re.search(r"(哪里|怎么买|价格|多少|链接|在哪|多少钱|贵吗)", comment_text):
            return random.choice(question_templates)
        if re.search(r"(教程|方法|怎么做|资料|步骤|流程)", comment_text):
            return random.choice(tutorial_templates)
