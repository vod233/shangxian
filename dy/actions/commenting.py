...
def _input_is_cleared(d, timeout=1.5):
    """检查评论输入框是否已被清空 — 这是评论发送成功的最强信号。"""
    deadline = time.time() + timeout
    while time.time() < deadline:
...
def _send_text_from_focused_input(d, text, log_label):
    """输入文本并发送，返回 True 表示发送成功。
    
    发送成功验证逻辑（优先级递减）：
    1. 输入框被清空（最强信号 — 抖音发送后自动清空输入框）
    2. 评论文字出现在页面可见评论中（次要信号）
    3. 文字不再存在于输入框中（辅助信号）
    """
    ...
    for attempt in range(3):
        ...
        if send_btn:
            ...
            logger.info(f"{log_label}: 已点击发送按钮 (第{attempt + 1}次)")
            time.sleep(random.uniform(0.8, 1.5))

            # 主验证：输入框是否被清空
            if _input_is_cleared(d):
                logger.info(f"{log_label}: ✅ 输入框已清空，确认发送成功")
                return True

            # 辅助验证：评论是否在页面可见 或 输入框中文字已变化
            if _message_visible(d, text):
                logger.info(f"{log_label}: ✅ 评论文字在页面可见，确认发送成功")
                return True
            if not _input_still_contains(d, text):
                logger.info(f"{log_label}: ✅ 输入框中文字已消失，确认发送成功")
                return True
    ...
    # 最终兜底：回车键发送
    logger.info(f"{log_label}: 发送按钮多次重试失败，尝试回车键发送")
    d.press("enter")
    time.sleep(random.uniform(1.0, 2.0))
    if _input_is_cleared(d) or _message_visible(d, text) or not _input_still_contains(d, text):
        logger.info(f"{log_label}: ✅ 回车键发送成功")
        return True

    logger.warning(f"{log_label}: ❌ 评论发送失败 — 输入框未被清空，文本未被发布")
    return False
...
class ProcessCommentSectionAction(BaseAction):
    ...
    def execute(self):
        ...
        swipe_count = 0
        reviewed_count = 0
        self._comment_made = False  # 本轮是否实际发送了评论
        ...
        return self._close_comment_section(), self._comment_made
    ...
    def _is_reviewable_comment(self, text):
        ...
        if len(clean) < 3:  # 与 ai_reply_agent.is_intent_comment 中的阈值一致
            return False
