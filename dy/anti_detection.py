...
class HumanGesture:
    ...
    def human_swipe_with_curve(cls, device, sx, sy, ex, ey, duration=None):
        ...
        # 每点间隔 10~40ms；总时长硬上限 400ms，留 100ms 安全余量避免触发长按(阈值 ~500ms)
        ...
        logger.debug(f"swipe_curve: points={len(points)} step={step_ms}ms total={total_ms}ms requested_dur={duration}s")
        try:
            device.swipe_points(points, duration=total_ms / 1000.0)
        except Exception:
            # 兜底：swipe_points 不可用时退回普通 swipe，总时长同样钳制在 400ms 内
            fallback_dur = min(duration, 0.40) if duration else 0.40
            device.swipe(sx, sy, ex, ey, duration=fallback_dur)

    @classmethod
    def fast_tap(cls, device, x, y, hold=0.045):
        """短按点击，降低抖音视频页把 click 误判成长按的概率。
        touch.up 失败会导致僵尸触控残留，后续操作可能触发长按菜单。"""
        try:
            device.touch.down(x, y)
            time.sleep(hold)
            device.touch.up(x, y)
        except Exception:
            # 显式释放可能残留的 touch down，防止僵尸触控累积
            try:
                device.touch.up(x, y)
            except Exception:
                pass
            # 兜底：完整 click 确保 touch 生命周期闭合
            try:
                device.click(x, y)
            except Exception:
                pass
...
class DailyLimitManager:
    ...
    - 线程锁保护 can_do/record_action 原子性，防止多设备竞态
    """
    ...
    def __init__(self, config=None, db_manager=None):
        ...
        # 线程锁：保护 can_do/record_action 原子性
        import threading
